...
- 日志行：一套字符选区（单行局部与跨行相同）。列间空白可被选中。Ctrl+A 整表 `visible`。右键走 `logs.row`（有选区复制选区文档，否则该行）。与 Ctrl+C 同一 `serializeLogCopy`。折叠徽章 `data-log-chrome` 不进选区。禁止在本页再挂 `YoContextMenu`。禁止把 Grid 碎片 `Selection.toString()` 当载荷。
