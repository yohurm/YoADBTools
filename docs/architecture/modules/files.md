...
- **清单列：** 表头与日志同一套 YoUI 列架：`YoColFrame` + `YoColRow` / `YoColHeader`。文件走 Family B：`cellPad=list`、`YoColTrack` / `YoColCell`、`YoVirtualList tone="list"`。日志走 Family A：`cellPad=list`（列垫进 `padLeftChars`）、ch 文档尺、VirtualList 默认 `document`（无行线）。列宽走 `col-model` / `col-resize`，模块只 `setColWidth`，不进设置
