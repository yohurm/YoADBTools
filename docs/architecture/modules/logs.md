...
- **清单列：** 表头是铬层，走 `YoColRow` + `YoColHeader`（元数据列可拖宽；消息列 `1fr`）。宽度代数在 YoUI `col-model` / `col-resize`，只影响表头，不驱动行几何。模块 store 只存 `colWidths` 并 `setColWidth`，不进设置。显示列仍读 `log_display_columns`，表头与文档段同一开关
- **行是文档（Family A，对照 Logcat Editor / VS Code Output / DevTools Console）：** 每行 DOM 文本 === `formatLogLine`（或显示列裁过的文档）。字段只着色，列对齐空格/冒号是文本节点，`white-space: pre`。不是 AG Grid / 文件清单那种单元格网格，禁止拖选中途切成「行块」
