...
- 过滤：UI `filter.ts` + domain `log_filter`（导出用）；共享 `core/yohu-domain/testdata/log_filter.json`。级别字母单源 `log_levels.json` ↔ `LEVELS`。`LogFilter.levels` 空 = 不限；非空 = 精确集合，禁止最低含以上。`tag_contains` 按逗号 / 分号 / `|` 拆多针，任一 OrdinalIgnoreCase **精确**命中（OR）；空或仅分隔符 = 不限。空白留在单个针内。无正则。逗号提交后前一针走 `YoChip`（写入盒 flex 行，关闭流内；禁止 Tag 槽嵌套横条）
- **级别钮：** 独立多选，走 `YoButton` `ink` + `flush`（不是 `YoSegmentedButton`）。槽写 `levelInkStyle`（只写 `--yohu-log-ink`）与 L1 `.yohu-ink`。选中只认 `aria-pressed`；inherit 按下 = 实底 `fill` + 字 `fg-on`。行反色仍只有 Fatal（行上 `data-paint=invert`）。禁止槽写 `data-paint` / `data-level`，禁止模块写 `--yohu-button-*` / color-mix / 六条软底。未选悬浮走 `--yohu-state-hover`。禁止 CSS 再列 V–F 映射，禁止点 `.yohu-button`。Tooltip 用公开 `stretch` 铺交叉轴，禁止点 `__anchor`
