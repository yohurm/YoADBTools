...
- **清单编辑器（三件套，对照 AS `logcat/messages/`）：** `editor/format` 一行一次 `formatMessage`（文本 + 着色 range + `headerChars`）；`editor/document` 只 `append` / `evict` / `reload` / `setOptions`；`editor/view` 软折行与 `YoVirtualList` 内聚，禁止回调 `formatMessage`。依赖：Formatter ↛ 任何显示模块；Document → Formatter；EditorView → Document。禁止 `doc.ts` / `wrap.ts` / `color/` 管道 / 投影器。panel 只喂 `ViewRow[]`（逻辑行 + 折叠/信号）；无新行不换 `visible` 引用
- **级别色：** 着色在 Formatter 写入时挂 range，不是第二次 paint，也不是 View if/else。身份在 `@yohu/api` `log-color-scheme` 目录（`log_color_scheme`，立即，默认 `yohu`）。Yohu 色值在 `tokens/colors.ts`；官方 AS Logcat V2 在 `tokens/logcat.ts`。级别是官方 LevelFormat：`" L "`（3ch）+ 1 个未着色空格，**不是**带 padLeft 的表格列。Yohu：tag / level / msg 共用 `--yohu-level-*`，Fatal level 是 `wash`（行盒），`bar=level`。Logcat：`bar=none`；Tag `abs(Java hashCode) % 80`。CSS 只认 `data-tone=ink|wash`、`data-box=line` 与 `data-bar`。清单行高 `dataRowHeight()` = `--yohu-row-height`。筛选钮仍走 `--yohu-level-*`。解析失败（`level=?`）整行只有消息。禁止 `--yohu-level-f-bg`、社区紫 Assert、模块再列 V–F hex、`data-tint-msg`
...
- **清单列：** 表头与行共用 Formatter `options` 一把尺（`trackTemplate`）。列序 `LOG_COLUMNS`：时间 / UID / PID / TID / Tag / 级别 / 消息。`YoColFrame` 默认 `cellPad=list`。轨道是 `(padLeft+chars+gutter)ch`。**级别例外**：`padLeft=0`、`chars=3`、`gutter=1`，Tag 在级别可见时 `gutter=0`，消息 `padLeft=0`。表头「级别」`YoColHeader pad="none"`，`resize=false`。`chPx` 只换 px 地板，不进 `formatOptionsKey`。store 只存 `colWidths`。表头 `For` 只遍历 `visibleLogColumns`（`LOG_COLUMNS` 稳定引用）。显示列仍读 `log_display_columns`（默认 UID/TID 关）。消息是 `line.msg` 原文，不加 `: `
- **软折行：** 属于 EditorView（对照 Editor SoftWrapModel），不改 Document。按行宽 − `headerChars` 切消息：硬 `\n` 强制切；其余优先空格后断，否则硬切。首行前缀永不折。续行 DOM 只有消息切片，`data-wrap` / `data-doc-from`；hang 是 `--yohu-log-hang` × `1ch`，禁止写进文档空格。VL 定高 = `dataRowHeight()`。禁止 `pre-wrap`
- **复制是清单文档（Family A）：** 载荷是 Document `text`。可视续行经 `data-doc-from` 映回逻辑偏移。未挂载中间行用同一 `text` 补齐。禁止 `Selection.toString()` 当跨行唯一载荷。Ctrl+A = 整表文档。折叠徽章 `data-log-chrome` 不进文档。选字底/字走 `--yohu-text-sel` / `--yohu-text-sel-fg`。列垫 `data-log-pad` 且 `user-select: none`
