...
- **清单列：** 官方 STANDARD 默认 `headerWidth=100`（时间 24 + BOTH 12 + Tag 24 + AppName 36 + Level 4）。列序时间 → UID（扩展，默认关）→ ProcessThread → Tag → AppName → Level → 消息。默认 PID+TID 开（`%5d-%-5d `）。显示列读 `log_display_columns`。消息是 `line.msg` 原文，不加 `: `。清单没有表格表头 / `1fr` 消息列。Tag 最大宽是官方 `TagFormat.maxLength` 常数（23），不经 store 列宽。`measureChPx` 给 wrap 算视口列数，给 clip 把文档 ch 换成 VL `contentWidth`（自然行宽横滑）
