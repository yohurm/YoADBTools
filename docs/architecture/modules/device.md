...
启动：`config/devices-catalog.json` 恢复上次成功目录，UI `device.list` 先画卡片；随后一趟 `start-server` + `devices -l` 对账。预热与 UI `device.refresh` 单飞。运行时 adb 只走 DataRoot 解压副本（或用户设置），安装包旁原件不解压成功后不再调用。恢复只用于冷启动首屏，成功空扫描仍整表清空。
