...
pub fn is_allowed_block_gap(ms: u64) -> bool {
...
mod tests {
    ...
    fn terminal_eval_request_is_id_values_serials() {
        ...
        assert!(is_allowed_block_gap(0));
        assert!(!is_allowed_block_gap(300));
    }
    ...
    fn command_group_dto_ignores_legacy_tags() {
        let parsed: CommandGroupDto = serde_json::from_str(
            r#"{"id":"g1","name":"设备信息","tags":["产线"],"entries":[]}"#,
        )
        .unwrap();
    ...
    fn library_entry_dto_tagged_command_and_block() {
        ...
        assert!(matches!(
            command,
            LibraryEntryDto::Command { ref id, .. } if id == "c1"
        ));
        ...
        .unwrap();
        match block {
            LibraryEntryDto::Block { gap_ms, steps, .. } => {
                assert_eq!(gap_ms, 1000);
                assert_eq!(steps[0].template, "shell echo 1");
            }
            LibraryEntryDto::Command { .. } => panic!("expected block"),
        }
    }
}
