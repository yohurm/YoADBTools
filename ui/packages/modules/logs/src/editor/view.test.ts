...
import { defaultFormatOptions, headerWidth } from "./format";
import { LineBoard, clipMessage, documentLines, visualBoardChars, wrapMessage } from "./view";
...
describe("documentLines / clip", () => {
  ...
  it("Soft-Wrap 关：续行文档含 hang 空格，可视 hang=0", () => {
    const message = formatted({ msg: "line1\nline2" });
    const visuals = documentLines(message);
    expect(visuals).toHaveLength(2);
    expect(visuals[0]?.text.endsWith("line1")).toBe(true);
    expect(visuals[1]?.hang).toBe(0);
    expect(visuals[1]?.text.startsWith(" ".repeat(message.headerChars))).toBe(true);
...
describe("LineBoard", () => {
  ...
  it("clip 无视行宽，长消息仍一行", () => {
    ...
    expect(visualBoardChars(first)).toBe(doc.messages[0]!.text.length);
