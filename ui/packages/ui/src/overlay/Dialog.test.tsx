...
function loadDialogCss(): string {
  const candidates = [
    resolve(process.cwd(), "src/overlay/Dialog.css"),
    resolve(process.cwd(), "packages/ui/src/overlay/Dialog.css"),
  ];
...
describe("YoDialog", () => {
  ...
  it("fit 外包 YoTravel，关窗冻锁，不自持行程引擎", () => {
    const candidates = [
      resolve(process.cwd(), "src/overlay/Dialog.tsx"),
      resolve(process.cwd(), "packages/ui/src/overlay/Dialog.tsx"),
    ];
