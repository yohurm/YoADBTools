...
function pathTree(): {
  ...
  const crumb = el("button", { "data-address": "crumb", class: "yohu-address__crumb" });
  const sep = el("span", { class: "yohu-address__crumb-sep" });
  const hit = el("button", { "data-address": "hit", class: "yohu-address__hit" });
  const crumbs = el("nav", { class: "yohu-address__crumbs" }, [crumb, sep, hit]);
  const input = el("input", { class: "yohu-address__field-input" });
  const field = el("div", { "data-address": "field", class: "yohu-address__field" }, [input]);
  const slot = el("div", { class: "yohu-address", "data-address": "slot" }, [crumbs, field]);
  const pad = el("div", { "data-address": "pad" });
  const path = el("div", { class: "yohu-files__path" }, [up, slot, pad]);
  return { path, up: upBtn, crumb, sep, hit, field, pad };
}

describe("address-field-model", () => {
