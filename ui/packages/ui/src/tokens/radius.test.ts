...
describe("圆角 / 间距 / 布局 token 契约", () => {
  ...
  it("布局常量与 theme.css 一致", () => {
    ...
    expect(Layout.ShellNav).toBe(200);
    expect(Layout.ShellNavIcons).toBe(Layout.TitlebarCaption);
    expect(cssVarValue("--yohu-layout-sidebar")).toBe(`${Layout.Sidebar}px`);
    expect(Layout.Sidebar).toBe(Layout.Preview);
    ...
    expect(cssVarValue("--yohu-layout-settings-max")).toBe(`${Layout.SettingsMax}px`);
    expect(Layout.SettingsMax).toBe(920);
    expect(cssVarValue("--yohu-layout-settings-number-w")).toBe(`${Layout.SettingsNumberW}px`);
    expect(Layout.SettingsNumberW).toBe(80);
    expect(cssVarValue("--yohu-layout-text-field-stepper")).toBe(`${Layout.TextFieldStepper}px`);
    expect(Layout.TextFieldStepper).toBe(Layout.IconSm);
    ...
    expect(cssVarValue("--yohu-layout-tooltip-arrow")).toBe(`${Layout.TooltipArrow}px`);
    expect(cssVarValue("--yohu-layout-tooltip-arrow-inset")).toBe(`${Layout.TooltipArrowInset}px`);
    expect(cssVarValue("--yohu-layout-tooltip-gap")).toBe(`${Layout.TooltipGap}px`);
